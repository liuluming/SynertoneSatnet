...
public class FragmentPageFive extends BaseFragment{

	private View view;
	private TextView mClientName, mBillTotle, mBillInfo,
			mLiuBillInfo, mBillInfoTv,mBillDate;
	private String mPageFiveToken,mCode,mResult,mYear,mMonth;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(SatnetApplication.accountModel!=null) {
			mPageFiveToken = SatnetApplication.accountModel.getSessionToken();
		}
        gson=new Gson();
		view = inflater.inflate(R.layout.persern_fragment_5, container,false);
		initView();
		isPrepared = true;
		lazyLoad();
		return view;
	...
	private void initView() {
		...
		if(month<=5){
			...
			mMonth= String.valueOf(month-5+12);
		}else{
			mYear= String.valueOf(year);
			mMonth= String.valueOf(month-5);
		}
		mClientName = view.findViewById(R.id.clint_name_lv);
		mBillTotle = view.findViewById(R.id.month_bill_totle_lv);
		mBillInfo = view.findViewById(R.id.bill_info_lv);
		mLiuBillInfo = view.findViewById(R.id.liulb_bill_info_lv);
		mBillInfoTv = view.findViewById(R.id.bill_info_tv);
		mBillDate = view.findViewById(R.id.tv_bill_date);
	...
	protected void lazyLoad() {
		if(!isPrepared || !isVisible) {
			...
			if (mPageFiveToken != null) {
				pageFiveRequestPost();
			}
		}
	}
	private void pageFiveRequestPost() {
		JSONObject jsonObject = new JSONObject();
		try {
			jsonObject.put("sessionToken", mPageFiveToken);
