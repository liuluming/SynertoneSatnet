...
public class FragmentPageFour extends BaseFragment{
	...
	private String mPageFourToken,mCode,mResult,mYear,mMonth;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(SatnetApplication.accountModel!=null) {
			mPageFourToken = SatnetApplication.accountModel.getSessionToken();
		}
        gson=new Gson();
		view = inflater.inflate(R.layout.persern_fragment_4, container,false);
		initView();
		isPrepared = true;
		lazyLoad();
		return view;
	...
	private void initView() {
		...
		if(month<=4){
			...
			mMonth= String.valueOf(month-4+12);
		}else{
			mYear= String.valueOf(year);
			mMonth= String.valueOf(month-4);
		}
	...
	protected void lazyLoad() {
		if(!isPrepared || !isVisible) {
			...
			if (mPageFourToken != null) {
				pageFourRequestPost();
			}
		}
	}
	private void pageFourRequestPost() {
		JSONObject jsonObject = new JSONObject();
		try {
			jsonObject.put("sessionToken", mPageFourToken);
