...
public class FragmentPageThree extends BaseFragment{
	...
	private String mPageThreeToken,mCode,mResult,mYear,mMonth;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(SatnetApplication.accountModel!=null) {
			mPageThreeToken = SatnetApplication.accountModel.getSessionToken();
		}
		gson=new Gson();
		view = inflater.inflate(R.layout.persern_fragment_3, container,false);
		initView();
		isPrepared = true;
		lazyLoad();
		return view;
	...
	private void initView() {
		...
		if(month<=3){
			...
			mMonth= String.valueOf(month-3+12);
		}else{
			mYear= String.valueOf(year);
			mMonth= String.valueOf(month-3);
		}
	...
	protected void lazyLoad() {
		if(!isPrepared || !isVisible) {
			...
			if (mPageThreeToken != null) {
				pageThreeRequestPost();
			}
		}
	}
	private void pageThreeRequestPost() {
		JSONObject jsonObject = new JSONObject();
		try {
			jsonObject.put("sessionToken", mPageThreeToken);
