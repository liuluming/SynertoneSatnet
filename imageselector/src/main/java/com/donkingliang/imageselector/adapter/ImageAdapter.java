...
public class ImageAdapter extends RecyclerView.Adapter<ImageAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        holder.ivSelectIcon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!chooseAble){
                    Toast.makeText(mContext,"上传中，请稍后操作！",Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    private boolean isFull() {
        if (isSingle && mSelectImages.size() == 1) {
            ...
            return mMaxCount > 0 && mSelectImages.size() == mMaxCount;
        }
    }
    ...
    public void unSelectImages() {
        Iterator<Image> iterator = mSelectImages.iterator();
