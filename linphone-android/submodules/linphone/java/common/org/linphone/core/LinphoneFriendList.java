...
public interface LinphoneFriendList {
	void setRLSAddress(LinphoneAddress addr);
	void setRLSUri(String uri);
	String getRLSUri();
	void addFriend(LinphoneFriend friend);
	void addLocalFriend(LinphoneFriend friend);
	LinphoneFriend[] getFriendList();
	void updateSubscriptions();
	void enableSubscriptions(boolean enable);
	LinphoneFriend findFriendByUri(String uri);
	...
	**/
    int importFriendsFromVCardFile(String file);

	/**
	 * Imports LinphoneFriends from a vCard 4 buffer
	 * @return the number of friend imported
	**/
    int importFriendsFromVCardBuffer(String buffer);
