...
public class HistoryListFragment extends Fragment implements OnClickListener, OnItemClickListener, ContactsUpdatedListener {
	...
	class CallHistoryAdapter extends  BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			TextView separatorText = view.findViewById(R.id.separator_text);
			...
			if(!TextUtils.isEmpty(userName)){
				if(!LinphoneUtils.localListPhones.contains(userName)){
					if(userName.startsWith("9")){
						userName=userName.substring(1);
					}else if(userName.startsWith("90")){
						userName=userName.substring(2);
					}
				}

			}
			if (c == null) c = ContactsManager.getInstance().findContactFromPhoneNumber(userName);
