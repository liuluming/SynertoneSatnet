...
public class CallOutgoingActivity extends LinphoneGenericActivity implements OnClickListener{
	...
	private static final int TIME_OUT=30120;
		...
		mListener = new LinphoneCoreListenerBase(){
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				if (call == mCall && State.Connected == state) {
					...
					finish();
					return;
				}else if(state==State.OutgoingRinging||state==State.OutgoingEarlyMedia){
					...
					if (call.getErrorInfo().getReason() == Reason.Declined) {
						...
						displayCustomToast(getString(R.string.no_answer_timeout), Toast.LENGTH_SHORT);
						decline();
					}else if(call.getErrorInfo().getReason()==Reason.BadCredentials){
						displayCustomToast(getString(R.string.error_credentials), Toast.LENGTH_SHORT);
						decline();
					}else if(call.getErrorInfo().getReason()==Reason.Unknown){
						displayCustomToast(getString(R.string.cant_accept_try_later), Toast.LENGTH_SHORT);
						decline();
					}
					...
					if (call.getErrorInfo().getReason() == Reason.Declined) {
						if(endTime-startTime>TIME_OUT){
							displayCustomToast(getString(R.string.no_respone), Toast.LENGTH_SHORT);
						}else{
							displayCustomToast(getString(R.string.error_call_declined), Toast.LENGTH_SHORT);
						}
