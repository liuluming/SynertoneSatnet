...
//        			}
	...
	private void initUI() {
		...
		RelativeLayout rl_message=findViewById(R.id.rl_message);
	...
	private void showVideoView() {
        if (!BluetoothManager.getInstance().isBluetoothHeadsetAvailable()&&!hasHeadset) {
...
//		Hiding controls to let displayVideoCallControlsIfHidden add them plus the callback
	...
	private void displayCurrentCall(LinphoneCall call){
		...
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				enabledVideoButton(LinphoneUtils.localListPhones.contains(name));
			}
		}, 320);
	}
