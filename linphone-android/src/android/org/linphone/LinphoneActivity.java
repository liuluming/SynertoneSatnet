...
public class LinphoneActivity extends LinphoneGenericActivity implements OnClickListener, ContactPicked, ActivityCompat.OnRequestPermissionsResultCallback {
	...
	private FragmentsAvailable lastFragmnet;
	...
	private void initView() {
		...
		menu_name.setText(getString(R.string.assistant));
	}
...
//		changeCurrentFragment(FragmentsAvailable.DIALER, extras);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == Activity.RESULT_FIRST_USER && requestCode == SETTINGS_ACTIVITY) {
			...
			boolean dialer = data != null && data.getBooleanExtra("dialer", false);
			if(chat){
				pendingFragmentTransaction = FragmentsAvailable.CHAT_LIST;
			}
			if(dialer){
				pendingFragmentTransaction = FragmentsAvailable.DIALER;
			}
	...
	protected void onStart() {
		super.onStart();
		/*AndPermission.with(LinphoneActivity.this)
				.requestCode(100)
				...
				.rationale(new RationaleListener() {
					...
					public void showRequestPermissionRationale(int requestCode, Rationale rationale) {
						AndPermission.rationaleDialog(LinphoneActivity.this, rationale)
								.show();
					}
				})
...
*/
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if (currentFragment==FragmentsAvailable.ACCOUNT_SETTINGS) {
				goToLastFragment();
				return true;
			}
		}
	...
	private void goToLastFragment() {
		if(isTablet()){
            if(lastFragmnet!=null){
                resetSelection();
                if(lastFragmnet ==  FragmentsAvailable.DIALER){
                    goToDialerFragment();
                }else if(lastFragmnet == FragmentsAvailable.HISTORY_DETAIL||lastFragmnet == FragmentsAvailable.HISTORY_LIST){
                    goToHistoryFragment();
                }else if(lastFragmnet == FragmentsAvailable.CONTACT_DETAIL||lastFragmnet==FragmentsAvailable.CONTACTS_LIST){
                    goToContactsFragment();
                }
            }
        }else{
	...
	public void initSideMenu() {
        ...
        sideMenuContent.setClickable(true);
...
https://github.com/liuluming/SynertoneSatnet0801.git
